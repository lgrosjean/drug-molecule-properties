...
class Dataset:
    ...
    def __init__(self, data_dir=None, mode='single', target_col='P1', smile_col='smiles', load=True, transform=True):
        if not mode in self.valid_mode:
            raise LoadError(f"Please provide a mode from {self.valid_mode} (provided: {mode})")
        self.mode=mode
