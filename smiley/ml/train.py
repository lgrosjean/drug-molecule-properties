...
class SingleModel:
    ...
    def __init__(self, input_shape:int, name:str='single_model', optimizer:str='adam', loss='binary_crossentropy', metrics:list=['accuracy'], weighted_metrics:list=None):
...
class SingleTrainer:
    ...
    def __init__(self, dataset=None, data_dir=None, params=None, load=True, transform=True):
        self.X = None
        self.y = None
        self.model = None
        self.epochs = 10
    ...
    def prepare(self, class_weight=True, random_state=54):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(self.X, self.y, test_size=self.test_size, stratify=self.y, random_state=self.random_state)
        self.X_train, self.X_val, self.y_train, self.y_val = train_test_split(self.X_train, self.y_train, test_size=self.val_size, stratify=self.y_train, random_state=self.random_state)
    ...
    def fit(self, X_train=None, y_train=None, validation_data=None, early_stopping:int=None, tqdm=True, scheduler=True, plot=True, **params):
        ...
        if early_stopping:
            ...
            es = early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss', patience=self.early_stopping)
